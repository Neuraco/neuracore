...
def test_get_latest_data_from_multiple_nodes():
    ...
    data_gatherer = nc.get_data_gatherer(
        robot_name=robot_name, instance=instance, include_remote=True
    )

    while data_gatherer.num_remote_nodes() == 0:
        time.sleep(0.25)

    while not data_gatherer.all_remote_nodes_connected():
        time.sleep(0.25)

    sync_point = data_gatherer.get_latest_data()
