...
from neuracore.core.nc_types import AvailableRobotInstance, RobotInstanceIdentifier
...
class StreamManagerFactory(BaseStreamManagerFactory):
    """A factory for creating stream managers for different robot instances.
    ...
    def get_consumer_manager(
        ...
        if key not in self.consumer_managers:
            self.consumer_managers[key] = ClientConsumerStreamManager(
                ...
                # todo add org nodes manager
                current_track_information=AvailableRobotInstance(tracks=[]),
                org_id=self.org_id,
                client_session=self.client_session,
                loop=self.loop,
                auth=self.auth,
...
_p2p_manager_factory: Optional[StreamManagerFactory] = None


def get_p2p_manager_factory() -> StreamManagerFactory:
    """Get the signalling message consumer.

    Returns:
        SignallingEventsConsumer: The singleton signalling message consumer instance.
    """
    global _p2p_manager_factory
    if _p2p_manager_factory is None:
        _p2p_manager_factory = StreamManagerFactory()
    return _p2p_manager_factory
