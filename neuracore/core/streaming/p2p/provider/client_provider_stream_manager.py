...
from .global_live_data_enabled import global_live_data_enabled_manager
...
class ClientProviderStreamManager(BaseP2PStreamManager):
    ...
    def __init__(
        ...
        self.background_tracker = BackgroundCoroutineTracker()

        self.streaming = EnabledManager(
            global_live_data_enabled_manager.is_enabled(), loop=loop
        )
        global_live_data_enabled_manager.add_listener(
    ...
    async def resubmit_tracks(self) -> None:
    ...
    def create_new_connection(
        ...
        for video_track in self.tracks:
            if connection_details.video_format == VideoFormat.WEB_RTC_NEGOTIATED:
                ...
                connection.add_event_source(video_track.get_neuracore_custom_track())
