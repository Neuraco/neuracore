...
from neuracore.core.streaming.p2p.stream_manager_factory import (
    StreamManagerFactory,
    get_p2p_manager_factory,
)
...
class OrgNodesManager(BaseSSEConsumer):
    ...
    def __init__(
        ...
        stream_manager_factory: Optional[StreamManagerFactory] = None,
    ) -> None:
        ...
        self.stream_manager_factory = (
            stream_manager_factory or get_p2p_manager_factory()
        )
    ...
    async def on_message(self, message_data: str) -> None:
        ...
        self.apply_stream_changes(self.last_nodes, robot_update)
    ...
    async def create_new_connection(
        ...
        response = await self.client_session.post(
            url=f"{API_URL}/org/{self.org_id}/signalling/connection",
            json=connection_request.model_dump(),
            headers=self.auth.get_headers(),
        )
        connection_message = HandshakeMessage.model_validate_json(await response.json())
