...
class DirectPolicy(Policy):
    ...
    def __init__(
        ...
        self._policy = PolicyInference(
            org_id=org_id, job_id=job_id, mar_file=model_path
        )
...
class ServerPolicy(Policy):
    ...
    def predict(self, sync_point: Optional[SyncPoint] = None) -> ModelPrediction:
        ...
        request_data = {"data": sync_point.model_dump()}
        if not self._is_local:
...
class LocalServerPolicy(ServerPolicy):
    ...
    def __init__(
        ...
        Args:
            ...
            job_id: Job ID
            model_path: Path to the .mar model file
        ...
        self.port = port
        self.host = host
        self._server_process: Optional[Popen] = None
        self._start_server()
    ...
    def _start_server(self) -> None:
        ...
        try:
            ...
            raise EndpointError(f"Failed to start local server: {str(e)}")

    def _wait_for_server(self, max_attempts: int = 30) -> None:
...
class RemoteServerPolicy(ServerPolicy):
    ...
    def __init__(
        self, robot: Optional[Robot], predict_url: str, headers: dict[str, str]
    ):
        """Initialize the remote server policy.

        Args:
            robot: Robot instance for accessing sensor streams.
            predict_url: URL of the remote prediction endpoint.
            headers: HTTP headers for authentication.
        """
        super().__init__(robot, predict_url, headers)


# Main connection functions
def launch_policy_direct(
    ...
    if train_run_name is not None:
        # Download model
        model_path = _download_model(train_run_name, org_id)
...
def launch_policy_local_server(
    train_run_name: str,
    port: int = 8080,
    robot_name: Optional[str] = None,
    instance: int = 0,
...
def launch_policy_remote_server(
    endpoint_name: str,
    robot_name: Optional[str] = None,
    instance: int = 0,
) -> RemoteServerPolicy:
...
# Helper functions
def _download_model(train_run_name: str, org_id: str) -> Path:
    """Download model from training run."""
    auth = get_auth()
    job_id = _get_job_id(train_run_name, org_id)
    destination = Path(tempfile.gettempdir()) / job_id / "model.mar"
    ...
    model_path = download_with_progress(
        ...
        f"Downloading model {train_run_name}",
...
def connect_endpoint(
    ...
    """Legacy compatibility wrapper for launch_policy_remote_server."""
    return launch_policy_remote_server(endpoint_name, robot_name, instance)
...
def connect_local_endpoint(
    ...
    """Legacy compatibility wrapper with both direct and server options."""
    ...
    if train_run_name:
        if port is None:
            return launch_policy_direct(train_run_name, robot_name, instance)
        else:
            return launch_policy_local_server(
                train_run_name, port, robot_name, instance
            )
    else:
        # Handle path_to_model case - for backward compatibility, implement basic logic
        raise NotImplementedError(
            "path_to_model support pending - use train_run_name instead"
        )
